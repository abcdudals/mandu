...
public class PageMaker {
	...
	private int epageNo; // view 에 표시할 마지막 PageNo
	private boolean prev; // 이전 PageBlock으로 이동
	private boolean next; // 다음 PageBlock으로 이동
	private int displayPageNo =3; // 1Page 장 표시할 pageNo 갯수
	private int lastPageNo; // 출력가능한 마지막 PageNo (totalRowCount, rowsPerPage 로 계산)
	...
	public void calcData() {
		...
		// => 계산의 결과로 얻어진 ePageNo 가 실제 lastPageNo 보다 크면 수정
		lastPageNo= (int)Math.ceil(totalRowCount/(double)cri.getRowsPerPage());
		// 예) 11/3 -> 결과 3,   11/(double)3 -> 결과 3.333
